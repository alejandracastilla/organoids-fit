...
#por alguna extra;a razon no corrio una pero no se cual, la ff si la corrio, toca probar cual se comio una a una
# pero lo hago en SRI porque ya casi es momento de la reunion
data_dd <- read_mrs(dd, format = "lcm_raw", ft=5.0028e8, fs = 10000, ref = 4.65)
fit_res_dd <- fit_mrs(data_dd, basis, opts=opts)
plot(fit_res_ee, main = 'fit dd')
