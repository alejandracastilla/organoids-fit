...
# -------------- devtools to install zoom in plots------------------------------------------------------------------
...
#---------------reference fit ala28_lcm-----
# #do we have concentration and value of ala28? Ans. Use the same as Ala29
ref_list <- list(ala2)
...
#---------------reference fit ala29_lcm-----
#reference sample fit
#volume = 30ul
#concentration = 20mM
# ref_list <- list(ala2)
# ref_basis <- sim_basis(ref_list, pul_seq = seq_pulse_acquire, 
#                        acq_paras = def_acq_paras(N = 8124, fs = 10000,
#                                                  ft = 5.0028e8, ref = 4.65, nuc="1H"))
# refname <- "../alanineRef/ala29_lcm"
# ref_data <- read_mrs(refname, format = "lcm_raw", ft=5.0028e8, fs = 10000, ref = 4.65)
#---------------abfit_opts-----------------
# opts <- abfit_opts(init_damping=5, #no deja 25, 
#                    maxiters=1024, #se revienta con el doble e incluso con 1800
#                    max_shift=0.078,
#                    max_damping=15,
#                    max_phase=360,
#                    lambda=NULL, #no deja ningun numero
...
#                    max_basis_damping = 2,
#                    maxiters_pre = 1000,
#                    algo_pre = "NLOPT_LN_NELDERMEAD", #NLOPT_LN_SBPLX lo simplifica mucho
#                    min_bl_ed_pppm = NULL, #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#                    max_bl_ed_pppm = 7,
#                    auto_bl_flex_n = 20,
...
#--------------- abfit opts----------------------------------
opts <- abfit_opts(ppm_left=4.8, pre_fit_bl_ed_pppm = 6.8, max_bl_ed_pppm = 14, auto_bl_flex_n = 5, algo_pre = "NLOPT_LN_SBPLX", auto_bl_flex=FALSE, init_damping=0)

# refit_phase_search = FALSE NOT DOING A DIFFFERENCE
#?opts
ref_res <- fit_mrs(ref_data, ref_basis, opts=opts)

#--------------- plotting reference fit ----------------------------------
plot(ref_res)
#plot(ref_res, xlim = c(3,4) 
plot(ref_res, xlim = c(3.3,3.6))
zm()
p <- ggplot(data = ref_res, aes (x = data, fill = clarity) )
ggplotly(p)
#---------------fitting data--------------------------------------------------
#---------------loading data-----
...
#---------------quantify concentrations using for in loop ----------------------
...
# attempt 2
# mass of 19 organoids in grames
# m_org <- c(... uknown mass of first organoids, 0.0113, 0.0158, 0.0091, 0.0039, 0.0098, 0.0043, 0.0076, 0.0089, 0.0038)
...
for (i in seq_along(fit_res)) {
  ...
  for (j in seq_along(metabs)) {
    ...
    conc <- (metabs[[j]]/ref_res$res_tab$Ala) * (0.02*0.03/vol_l[[index]])
    output[i, j] <- conc
  }
  index <- (index+1)
}
#---------------saving table with names of organoids and metabolites----------- -----------
...
rownames(df)  <- c("b", "c", "d", "e", "f", "g", "h", "i", "k", "l", "s", "t", "u", "aa", "bb", "cc", "dd", "ee", "ff")
#write data frame to csv file
write.csv(df,"../csv/output-May8.csv", row.names = TRUE)
...
statsdf$Age <- c(103, 104, 121, 122, 123, 136, 137, 138, 185, 184, 311, 312, 313, 85, 86, 87, 144, 145, 146)
